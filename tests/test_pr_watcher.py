...
async def test_new_human_comment_queues_rework(monkeypatch):
    ...
    patch(
        ...
        pr_comments=make_async(
            [
                IssueComment(
                    author="alice", body="Please rename the button", created_at="2024-06-01T10:00:00Z"
                ),
            ]
        ),
        comment_pr=comment_pr,
    ...
    patch(
        ...
        pr_comments=make_async(
            [
                IssueComment(
                    author="a",
                    body="revert these lines",
                    created_at="2024-06-01T10:00:00Z",
                    path="e2e/version.spec.ts",
                    line=8,
                    diff_hunk="@@ -1 +8 @@",
                ),
                IssueComment(
                    author="a",
                    body="rename just the test name",
...
async def test_bot_comment_is_ignored(monkeypatch):
    ...
    patch(
        ...
        pr_comments=make_async(
            [
                IssueComment(
                    author="bot", body="🤖 **agent-loop:** opened a PR", created_at="2024-06-01T10:00:00Z"
                ),
            ]
        ),
    )
